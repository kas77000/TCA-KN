...
# -*- coding: utf-8 -*-
"""
TCA deck builder v2 — the short pack
============================================

Same data and same analysis as tca_deck.py, but built as a FOUR-slide pack for
a short meeting:

    1  The number, and what we see
    2  Where the money is made and lost
    3  What explains it
    4  What we would change

Slide 3 is chosen from the data rather than fixed. Where one country-and-side
cell is worth more than half the whole result, that cell IS the story and leads.
Otherwise the venue mix leads, because that is what decides the fix.

    python tca_deck_v2.py --client KIC
    python tca_deck_v2.py --client NPS --slides 5

No order file is needed - everything comes from the published report figures in
CLIENTS. Use --simple for the longer pack that tca_deck.py produces.
============================================
...
def pick(items, *tags, limit=None):
    """Text of the narrative items carrying these tags, in the order asked for.

    Selecting on the tag rather than the wording means rewriting a sentence
    cannot quietly drop it from a slide.
    """
    out = []
    for t in tags:
        for it in items or []:
...
def texts(items, limit=None):
    """Plain text of every item, in order."""
    out = [it.get("text", "") for it in (items or [])]
    return out[:limit] if limit else out


def pick_lead_exhibit(ctx: dict) -> str:
    """Which single chart carries this client's story.

    If one country-and-side cell is worth more than half the headline, the
    result IS that cell and nothing else competes. Otherwise the venue mix is
    the exhibit that tells you what to change, so it leads.
    """
    hl = ctx["headline"]
    S = ctx.get("side_lens")
    if S and S.get("worst") is not None and np.isfinite(hl.get("pnl_ccy", np.nan)):
        share = abs(S["worst"]["pnl_ccy"]) / max(abs(hl["pnl_ccy"]), 1e-9)
        if share >= 0.5 and ctx["charts"].get("side"):
            return "side"
    if ctx["charts"].get("venue_country"):
        return "venue_country"
    if ctx["charts"].get("side"):
        return "side"
    return "attribution"


def sh_headline(prs, ctx, nar):
    s = new_slide(prs)
    hl = ctx["headline"]
    band = s.shapes.add_shape(1, Inches(0), Inches(0), SLIDE_W, Inches(1.95))
    band.fill.solid(); band.fill.fore_color.rgb = T_HEAD
    band.line.fill.background(); band.shadow.inherit = False
    stripe = s.shapes.add_shape(1, Inches(0), Inches(1.95), SLIDE_W, Inches(0.14))
    stripe.fill.solid(); stripe.fill.fore_color.rgb = T_ACCENT
    stripe.line.fill.background(); stripe.shadow.inherit = False

    box = s.shapes.add_textbox(MARGIN, Inches(0.42), Inches(11.5), Inches(1.3))
    tf = box.text_frame; tf.word_wrap = True
    _run(tf.paragraphs[0], "Transaction Cost Analysis", size=32, bold=True,
         color=T_WHITE)
    p2 = tf.add_paragraph()
    _run(p2, f"{CLIENT_NAME}  ·  {PERIOD_LABEL}", size=15, color=T_WHITE)
    ...
    left = MARGIN
    for big, cap_, fill in cards:
        kpi(s, left, Inches(2.32), w, Inches(1.05), big, cap_, fill=fill,
            big_size=21 if len(big) <= 10 else 16)
        left += w + gap

    box = s.shapes.add_textbox(MARGIN, Inches(3.70), Inches(12.1), Inches(0.3))
    _run(box.text_frame.paragraphs[0], "WHAT WE SEE", size=11, bold=True,
         color=T_HEAD)
    lead = pick(nar["findings"], "headline", "algo", "market_good",
                "market_bad", "side_worst")
    bullets(s, lead[:5], MARGIN, Inches(4.02), Inches(12.1),
            Inches(2.5), size=11.5)
    note(s, "1 bps = 0.01%. A plus means you saved money against the reference "
            "price, a minus means it cost you. Every figure is from your "
            "published post-trade report.")
    notes(s, f"""WHAT THIS SLIDE SHOWS
The score for the whole period, and the handful of things behind it.

WORDS USED HERE
"bps" means basis points. 1 bps = 0.01%, so 100 bps = 1%. We use it because
these differences are small next to the amount of money traded.

"Benchmark" is a fair reference price. For most orders it is the average price
everyone else paid while your order was running. Beating it means you saved
money.
...
Four slides. This one is the score. The next shows which countries drove it.
The third shows why. The last is what we would change.""")
    footer(s)


def sh_where(prs, ctx, nar):
    s = new_slide(prs)
    title(s, "Where the Money Is Made and Lost")
    strapline(s, "A country only moves your total if you did well or badly "
                 "there AND you traded a lot there.")
    picture(s, ctx["charts"].get("attribution"), MARGIN, Inches(1.52),
            width=Inches(7.4))

    lines = pick(nar["findings"], "market_good", "market_bad", "industry")
    mkt = ctx["market_table"]
    if not mkt.empty:
        top3 = mkt.nlargest(3, "notional_m")
        lines.append("Your three biggest markets — **"
                     ...
                     + f"** — are {top3['weight_pct'].sum():.0f}% of everything "
                       "you trade.")
    bullets(s, lines, Inches(8.15), Inches(1.72), Inches(4.6), Inches(4.5),
            size=10.5)
    notes(s, """WHAT THIS SLIDE SHOWS
Which countries helped your result and which hurt it.

HOW TO READ IT
The bar is not simply how well you traded there. It is how well you traded
there multiplied by how much you traded there.

That distinction matters. A country can be terrible but tiny and barely affect
your total. Another can be only slightly bad but enormous and dominate it. The
line under each country name shows both halves.

Blue adds to your result, red takes away. Add every bar together and you get
the headline number from slide 1.

WHY IT MATTERS
It tells you where your attention is worth spending, which is not the same as
where the worst percentage is.""")
    footer(s)


def sh_why(prs, ctx, nar):
    s = new_slide(prs)
    lead = pick_lead_exhibit(ctx)

    if lead == "side":
        title(s, "What Explains It")
        strapline(s, "The same money, split into what you bought and what you "
                     "sold. This is where the number really sits.")
        picture(s, ctx["charts"]["side"], MARGIN, Inches(1.50),
                width=Inches(7.1))
        lines = pick(nar["findings"], "side_worst", "side_best", "side_widest")
        note_txt = """WHAT THIS SLIDE SHOWS
The same money as the last slide, but split by whether you were buying or
selling.

HOW TO READ IT
Each row is one country and one direction. "Japan - BUY" is every share you
bought in Japan over the period. The bars are in dollars, so the longest bar is
genuinely the largest amount of money.

WHY THIS IS THE SHARPEST VIEW
Buying and selling in the same country often go opposite ways and cancel each
other out everywhere else. A country can look calm overall while hiding a big
loss on one side and a big gain on the other. Splitting them is what lets you
point at one specific pocket instead of a whole country.

ONE THING TO CHECK
A gap between buying and selling that lasts months is often about which way the
market moved rather than how you traded. If prices rose all period and you were
mostly buying, buying will look worse. Worth confirming before changing
anything."""
    else:
        title(s, "What Explains It")
...
THE FOUR WAYS
AUCTION (blue) - the single batch trade at the open or close. Everyone gets one
price.

POSTED (green) - you leave your order waiting and let others come to you. You
earn the gap between the buy and sell price for being patient. The risk is not
getting filled.

CROSSING THE SPREAD (orange) - you take the price on offer right now. Immediate
and certain, but you pay away roughly half that gap every time.

DARK (purple) - a private venue where both sides meet in the middle. You
neither pay the gap nor earn it.

HOW TO READ IT
Each bar is 100% of what you traded in that country. Broadly, more green and
purple is better; more orange means you are paying the gap away.

WHY THIS DECIDES THE FIX
A country that loses money AND crosses a lot can be improved by changing
settings - that is in your control. A country that loses money but already
posts and already uses the midpoint cannot be fixed that way; its problem is
how fast the orders run, not where they go."""

    lines += texts(nar["recs"], limit=2)
    bullets(s, lines, Inches(8.05), Inches(1.70), Inches(4.7), Inches(4.6),
            size=10.5)
    notes(s, note_txt)
    footer(s)
...
def sh_extra(prs, ctx, nar):
    """The fifth slide, when five are allowed: algo and order size together."""
    s = new_slide(prs)
    title(s, "By Algorithm and Order Size")
    strapline(s, "Which tools you use, and whether small or large orders do "
                 "better.")
    picture(s, ctx["charts"].get("algo"), MARGIN, Inches(1.50),
            width=Inches(6.3))
    picture(s, ctx["charts"].get("adv"), Inches(7.05), Inches(1.50),
            width=Inches(5.75))
    lines = pick(nar["findings"], "algo", "size", "cap")
    bullets(s, lines, MARGIN, Inches(4.85), Inches(12.1), Inches(1.6),
            size=10.5)
    notes(s, """WHAT THIS SLIDE SHOWS
Left: how each trading algorithm performed. Right: whether your small orders or
your large ones get the better result.

WORDS USED HERE
An "algorithm" is the automated strategy that works your order for you. VWAP
spreads an order through the day. IIS aims for the closing price.

"% of a normal day's trading" is how big your order is compared with how much
that share usually trades in a day. 1% is easy to hide, 10% is not.

WATCH OUT
An algorithm showing exactly 0.0 is not perfect - it trades at the closing
price and is measured against the closing price, so it is compared with itself
and always scores zero. It tells us nothing about how well it did.

Normally big orders cost more. If your small ones do worse, the easy flow is
being pushed too fast - and easy flow is usually most of your money.""")
    footer(s)


def sh_change(prs, ctx, nar):
    s = new_slide(prs)
    title(s, "What We Would Change")
    strapline(s, "Biggest win first.")
    for i, r in enumerate(texts(nar["recs"], limit=4)):
        y = Inches(1.60 + i * 1.02)
        num = s.shapes.add_shape(1, MARGIN, y, Inches(0.42), Inches(0.42))
        num.fill.solid(); num.fill.fore_color.rgb = T_HEAD
        num.line.fill.background(); num.shadow.inherit = False
        tf = num.text_frame; tf.margin_top = Inches(0.02)
        tf.paragraphs[0].alignment = PP_ALIGN.CENTER
        _run(tf.paragraphs[0], str(i + 1), size=14, bold=True, color=T_WHITE)
        bullets(s, [r], Inches(1.18), y - Inches(0.05), Inches(11.4),
                Inches(0.92), size=11.5, marker="")
    if nar["notes"]:
        box = s.shapes.add_textbox(MARGIN, Inches(5.80), Inches(12.1), Inches(0.3))
        _run(box.text_frame.paragraphs[0], "WORTH KNOWING", size=10, bold=True,
             color=T_MUTED)
        bullets(s, ["~" + x for x in texts(nar["notes"], limit=3)], MARGIN,
                Inches(6.10), Inches(12.1), Inches(0.85), size=9, gap=2)
    notes(s, """WHAT THIS SLIDE SHOWS
What we would actually change, in order of what it is worth.

HOW TO READ IT
Each item names a specific change and roughly what it is worth in money. They
are ordered by size, so number 1 is where to start. We have avoided "look into
X" - everything here can be acted on.
...
def build_short_deck(ctx: dict, nar: dict, out: Path, n_slides=4) -> Path:
    _PAGE[0] = 0
    prs = Presentation()
    prs.slide_width, prs.slide_height = SLIDE_W, SLIDE_H
    sh_headline(prs, ctx, nar)
    sh_where(prs, ctx, nar)
    if n_slides >= 5:
        sh_extra(prs, ctx, nar)
    sh_why(prs, ctx, nar)
    sh_change(prs, ctx, nar)
    prs.save(str(out))
    log(f"  saved {out.name}  ({len(prs.slides._sldIdLst)} slides)")
    return out
...
def main(argv=None) -> int:
    ...
    if args.simple or args.short or not args.data:
